/*
 * Lab 5: PDA for wcw^r — Accepted by Final State
 *
 * Language: { wcw^r | w in {0,1}* }
 *   w  = any string over {0,1}
 *   c  = centre marker
 *   w^r = reverse of w
 *
 * PDA Definition:
 ...
 * +-------+-------+-----------+-------------------------+
 *
 * Phase 1 (state q0): Push every input symbol onto stack.
 * Phase 2 (after 'c', state q1): Pop and match each input symbol.
 * Accept : Reach *q2 with Z0 on stack top.
 */
    ...
    for (i = 0; i < len; i++) {
        ...
        if (state == 0) {
            ...
            if (inp == '0' && st == 'Z') {
                ...
                /* Transition on 'c': move to q1, stack unchanged */
